 ...
 *
 * The version of the OpenAPI document: v4.0.17.2952
 * 
...
pub async fn create_quality_profile(configuration: &configuration::Configuration, quality_profile_resource: Option<models::QualityProfileResource>) -> Result<models::QualityProfileResource, Error<CreateQualityProfileError>> {
    ...
    let p_body_quality_profile_resource = quality_profile_resource;
    ...
    req_builder = req_builder.json(&p_body_quality_profile_resource);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::QualityProfileResource`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::QualityProfileResource`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn delete_quality_profile(configuration: &configuration::Configuration, id: i32) -> Result<(), Error<DeleteQualityProfileError>> {
    ...
    let uri_str = format!("{}/api/v3/qualityprofile/{id}", configuration.base_path, id=p_path_id);
...
pub async fn list_quality_profile(configuration: &configuration::Configuration, ) -> Result<Vec<models::QualityProfileResource>, Error<ListQualityProfileError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::QualityProfileResource&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::QualityProfileResource&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn update_quality_profile(configuration: &configuration::Configuration, id: &str, quality_profile_resource: Option<models::QualityProfileResource>) -> Result<models::QualityProfileResource, Error<UpdateQualityProfileError>> {
    ...
    let uri_str = format!("{}/api/v3/qualityprofile/{id}", configuration.base_path, id=crate::apis::urlencode(p_path_id));
