...
/// struct for typed errors of method [`get_localization_by_id`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetLocalizationByIdError {
    UnknownValue(serde_json::Value),
}
...
pub async fn get_localization(configuration: &configuration::Configuration, ) -> Result<models::LocalizationResource, Error<GetLocalizationError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::LocalizationResource`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::LocalizationResource`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_localization_by_id(configuration: &configuration::Configuration, id: i32) -> Result<models::LocalizationResource, Error<GetLocalizationByIdError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;

    let uri_str = format!("{}/api/v3/localization/{id}", configuration.base_path, id=p_path_id);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetLocalizationByIdError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
