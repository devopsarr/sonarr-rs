...
/// struct for typed errors of method [`create_series_import`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateSeriesImportError {
...
pub async fn create_series_import(configuration: &configuration::Configuration, series_resource: Option<Vec<models::SeriesResource>>) -> Result<(), Error<CreateSeriesImportError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let entity: Option<CreateSeriesImportError> = serde_json::from_str(&content).ok();
